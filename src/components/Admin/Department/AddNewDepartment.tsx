...
const AddNewDepartment = ({ closeModal, refetch }: IProp) => {
  const {
    register,
    handleSubmit,
    formState: { errors }
  ...
  const addNewDepartmentQuery = useMutation(createDepartment, {
  ...
  const onSubmit: SubmitHandler<FromType> = async (data) => {
    addNewDepartmentQuery.mutate(data)
  }
  ...
  return (
    ...
    >
      ...
      <div className='w-full flex justify-end'>
        ...
        >
          {addNewDepartmentQuery.isLoading ? <LoadingIcon /> : 'Thêm'}
        </button>
      </div>
    </form>
  )
